  ...
  return (
    <div className="pb-32 max-w-md mx-auto px-4 pt-3">
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-1.5 text-[11px] font-semibold text-teal-800 bg-teal-50 border border-teal-200/60 px-2.5 py-1 rounded-full shadow-2xs">
      ...
      {/* User Info Card */}
      <div className="p-5 rounded-3xl bg-white border border-slate-200/80 shadow-md shadow-teal-900/5 flex items-center gap-4">
        <div className="w-16 h-16 rounded-full overflow-hidden bg-teal-50 border-2 border-teal-200 flex items-center justify-center shrink-0">
          {user?.avatar ? (
            <img
              src={user.avatar}
              alt={user.name}
        ...
        <div className="leading-tight flex-1">
          <h2 className="text-base font-bold text-slate-900">{user?.name || 'SwiftScan Member'}</h2>
          <div className="flex items-center gap-1.5 text-xs text-slate-500 mt-1">
            <Mail className="w-3.5 h-3.5 text-teal-600" />
            <span className="truncate">{user?.email || 'member@swiftscan.co'}</span>
          </div>
          <div className="flex items-center gap-1.5 mt-2">
            <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-teal-50 text-teal-800 border border-teal-200/60 shadow-2xs">
      ...
      <div className="mt-4 p-5 rounded-3xl bg-gradient-to-br from-teal-800 to-teal-950 text-white shadow-lg shadow-teal-950/20">
        <div className="flex justify-between items-start mb-3">
          <div>
            <span className="text-[10px] tracking-wider uppercase font-bold text-teal-300">
              In-Store Membership
            </span>
            <div className="text-sm font-extrabold">Fast-Lane Pass</div>
          </div>
        </div>

        {/* Member Barcode */}
        <div className="bg-white p-2.5 rounded-2xl flex flex-col items-center shadow-xs">
      ...
      <div className="mt-4 p-4 rounded-3xl bg-white border border-slate-200/80 shadow-md shadow-teal-900/5 text-xs">
