...
const slides: Slide[] = [
  {
    ...
    renderIllustration: () => (
      <div className="relative w-64 h-64 mx-auto flex items-center justify-center">
        ...
        <div className="absolute inset-0 rounded-full bg-[#8BA89F]/15 blur-xl transform scale-90" />
        ...
        <div className="relative w-44 h-56 rounded-3xl bg-white border-4 border-slate-800 shadow-xl overflow-hidden flex flex-col p-2.5">
          ...
          <div className="relative flex-1 rounded-2xl bg-slate-900 overflow-hidden flex items-center justify-center">
            {/* Viewfinder corners */}
            <div className="absolute top-3 left-3 w-4 h-4 border-t-2 border-l-2 border-[#8BA89F] rounded-tl" />
            <div className="absolute top-3 right-3 w-4 h-4 border-t-2 border-r-2 border-[#8BA89F] rounded-tr" />
            <div className="absolute bottom-3 left-3 w-4 h-4 border-b-2 border-l-2 border-[#8BA89F] rounded-bl" />
            <div className="absolute bottom-3 right-3 w-4 h-4 border-b-2 border-r-2 border-[#8BA89F] rounded-br" />
            ...
            <div className="w-16 h-20 rounded-xl bg-slate-800 border border-slate-700 flex flex-col items-center justify-center p-1.5 shadow-md">
              <div className="w-8 h-8 rounded-lg bg-[#8BA89F]/30 flex items-center justify-center mb-1">
                <div className="w-4 h-4 rounded-full bg-[#8BA89F]" />
              </div>
            ...
            <motion.div
              ...
              className="absolute inset-x-3 h-[2px] bg-[#F2A68D] shadow-[0_0_10px_#F2A68D]"
            />
          </div>

          <div className="mt-2 flex items-center justify-between px-1">
            <div className="w-14 h-2 bg-slate-200 rounded" />
            <div className="w-6 h-2 bg-[#8BA89F] rounded" />
          </div>
        </div>
        ...
        <motion.div
          ...
          className="absolute -bottom-1 -right-2 bg-white px-3 py-1.5 rounded-full shadow-lg border border-slate-100 flex items-center gap-1.5 text-xs font-semibold text-[#4A635B]"
        >
          <div className="w-2 h-2 rounded-full bg-[#8BA89F] animate-ping" />
  ...
  },
  {
    ...
    renderIllustration: () => (
      <div className="relative w-64 h-64 mx-auto flex items-center justify-center">
        <div className="absolute inset-0 rounded-full bg-[#F2A68D]/15 blur-xl transform scale-90" />
        
        {/* Shopping Tote Illustration */}
        <div className="relative w-48 h-48 flex items-center justify-center">
          {/* Tote Bag Body */}
          <div className="w-40 h-36 rounded-b-3xl rounded-t-lg bg-gradient-to-b from-[#8BA89F] to-[#729188] relative shadow-lg flex flex-col justify-end p-4 border border-[#8BA89F]/40">
            {/* Bag Handles */}
            <div className="absolute -top-12 left-1/2 -translate-x-1/2 w-20 h-16 border-4 border-[#4A635B] rounded-t-full bg-transparent" />
        ...
        <motion.div
          ...
          className="absolute top-2 right-4 bg-white px-3 py-1 rounded-full shadow-md text-xs font-bold text-[#4A635B] border border-slate-100"
        >
          3 Items in Tote
        </motion.div>
      </div>
    ...
    renderIllustration: () => (
      <div className="relative w-64 h-64 mx-auto flex items-center justify-center">
        ...
        <div className="relative w-44 rounded-2xl bg-white border border-slate-100 shadow-xl p-3 flex flex-col items-center">
          {/* Receipt Header */}
          <div className="w-7 h-7 rounded-full bg-[#8BA89F]/20 flex items-center justify-center text-[#4A635B] mb-2">
          ...
          <div className="w-full space-y-1 text-[10px] text-slate-600">
            ...
            <div className="flex justify-between font-bold text-slate-900 pt-1 border-t border-slate-100">
              ...
              <span className="text-[#4A635B]">$10.38</span>
            </div>
          </div>

          {/* Exit Gate Barcode */}
          <div className="mt-2.5 p-1.5 bg-[#F9FAFB] rounded-xl w-full flex flex-col items-center">
...
export const OnboardingCarousel: React.FC<OnboardingCarouselProps> = ({ onComplete }) => {
  ...
  return (
    <div className="min-h-screen w-full bg-[#F9FAFB] flex flex-col justify-between p-6 max-w-md mx-auto relative overflow-hidden select-none">
      ...
      <div className="flex items-center justify-between pt-2">
        ...
        {!isLast ? (
          <button
            ...
            className="text-xs font-semibold text-[#4A635B] hover:text-slate-800 transition-colors py-1 px-3 rounded-full hover:bg-slate-100"
          >
            Skip
          </button>
        ) : (
      ...
      <div className="flex-1 flex flex-col items-center justify-center my-6">
        <AnimatePresence mode="wait">
          ...
          >
            ...
            {/* Pill Badge */}
            <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-[#8BA89F]/15 text-[#4A635B] mb-3">
              {slides[currentIndex].badge}
            </span>

            {/* Title & Subtitle */}
            <h2 className="text-2xl font-bold tracking-tight text-slate-800 leading-snug max-w-xs">
      ...
      <div className="pb-4 space-y-6">
          ...
          {slides.map((_, idx) => (
            <button
              ...
              className={`h-2 rounded-full transition-all duration-300 ${
                idx === currentIndex ? 'w-7 bg-[#8BA89F]' : 'w-2 bg-slate-200'
              }`}
            />
          ))}
        </div>
        ...
        <button
          ...
          onClick={handleNext}
          className="w-full py-3.5 px-6 rounded-3xl bg-[#F2A68D] hover:bg-[#e8957a] text-white font-semibold text-sm shadow-sm transition-all duration-200 active:scale-[0.98] flex items-center justify-center gap-2"
        >
